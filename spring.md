...
Щгромное количество Web - приложений в интернете работают на Spring MVC.
...
Конфигурационный файл Spring должен называться __applicationComtext.xml__
...
+ Вя конфигурация на Java коде (современный способ).
...
Еще один scope, который называется __Prototype__, каждый раз раз создает новый объект при вызове `getBean()`. Используется, когда у бина есть изменяемые состояния (stateful).
